# Phase 2 bake-off: XML.jl-backed prototype loader vs the saved baseline, same methodology as
# run_loader_bench.jl (Chairmarks @be, warm median + rel-sigma, saved to bench/results/*.json).
# Correctness already verified separately (prototype_xmljl/validate.jl - 18/18 small fixtures +
# the large synthetic fixture match the real EzXML-backed loader byte-for-byte).
...
println("XML.jl-prototype loader bake-off — warm median, Chairmarks @be.")
...
for (module_dir, xml_files) in generic_test_files
    ...
    for xml_path in xml_files
        ...
        b = @be XmlJLLoaderProto.load($xml_path, $module_ref) seconds = 2
        r = stats(b)
...
b = @be XmlJLLoaderProto.load($large_xml, $large_module_ref) seconds = 5
r = stats(b)
n_entries = length(XmlJLLoaderProto.load(large_xml, large_module_ref).Entry)
println(
...
mkpath(resdir)
resfile = joinpath(resdir, "loader_bakeoff_xmljl.json")
