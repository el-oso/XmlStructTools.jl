# Phase 2 bake-off: quick-xml-shim-backed prototype loader vs the saved baseline. Same
# methodology as run_loader_bench.jl / run_bakeoff_xmljl.jl / run_bakeoff_pugixml.jl.
...
println("quick-xml-prototype loader bake-off — warm median, Chairmarks @be.")
...
for (module_dir, xml_files) in generic_test_files
    ...
    for xml_path in xml_files
        ...
        b = @be QuickxmlLoaderProto.load($xml_path, $module_ref) seconds = 2
        r = stats(b)
...
b = @be QuickxmlLoaderProto.load($large_xml, $large_module_ref) seconds = 15
r = stats(b)
n_entries = length(QuickxmlLoaderProto.load(large_xml, large_module_ref).Entry)
println(
...
mkpath(resdir)
resfile = joinpath(resdir, "loader_bakeoff_quickxml.json")
